...
in an easy and intuitive way. The functions require a formula in the same way and format as `lm` which
is a style most R users are familiar with.
...
* colf_nlxb - uses Nash's variant of Marquardt nonlinear least squares solution which comes from the 
  `nlmrt::nlxb` function.
...
run a constrained least squares optimization problem.

In the model below we use 4 variables to estimate mpg which means we will have 5 parameter 
estimates (don't forget the Intercept). Parameters are prefixed with `param_` in the formula. We 
set the lower bounds of those 4 parameter estimates to -2 and the upper bounds to 2 
...
do not fall within the boundaries then an error will be returned and you would need to manually change 
the starting values via the `start` argument.
